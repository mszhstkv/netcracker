...
const incidentReducer = (state = initState, action: ActionsType): InitialStateType => {
    switch (action.type) {
        case SET_USERS:
            return {
                ...state,
...
type EditIncidentType = {
    ...
    _id: string, incidentTitle: string, assignee: string, area: string,
    startDate: moment.Moment, dueDate: moment.Moment, description: string,
    priority: string, status: string
};

export const setIncidentFormCreateValues = (value: IncidentsType): SetIncidentFormCreateValuesActionType => ({
    type: INCIDENT_FORM_CREATE_VALUES,
    value
});
export const setIncidentFormEditValues = (value: IncidentsType): SetIncidentFormEditValuesActionType => ({
    type: INCIDENT_FORM_EDIT_VALUES,
    value
});
export const setUsers = (users: Array<UsersType>): SetUsersActionType => ({type: SET_USERS, users});
export const setIncidents = (incidents: Array<IncidentsType>): SetIncidentsActionType => ({
    type: SET_INCIDENTS,
    incidents
});
export const setIncidentIsLoading = (incidentIsLoading: boolean): SetIncidentIsLoadingActionType => ({
...
export const create = (incidentTitle: string, assignee: string, area: string,
                       startDate: moment.Moment, dueDate: moment.Moment, description: string,
                       priority: string, status: string): CreateType =>
    ({
        type: GET_CREATE, incidentTitle, assignee, area, startDate, dueDate, description, priority, status
    });
export const deleteIncident = (id: string): DeleteIncidentType => ({type: DELETE_INCIDENT, id});
export const editIncident = (_id: string, incidentTitle: string, assignee: string, area: string,
                             startDate: moment.Moment, dueDate: moment.Moment, description: string,
                             priority: string, status: string): EditIncidentType =>
    ({
        type: PUT_EDIT_INCIDENT, _id, incidentTitle, assignee, area, startDate, dueDate, description, priority, status
    });
